...
import { kickerFor, DISCLAIMERS } from '../../resources/js/sections.mjs';
...
export function renderArticle(f) {
  ...
  const kind = DISCLAIMERS[f.disclaimer];
  const disclaimer = (kind || f.hasDisclaimer) ? `
        <aside class="callout callout--${kind ? kind.variant : 'ai'}">
          <span class="callout__icon" aria-hidden="true">▲</span>
          <p>${esc(kind ? kind.text : (f.disclaimerText || DISCLAIMERS['ai-market'].text))}</p>
        </aside>` : '';
  return `<!DOCTYPE html>
