...
test('classifyUrl maps each directory to its type and section', () => {
  assert.deepEqual(classifyUrl('/gojo/stocks/spy-x.html'), { type: 'market-take', section: 'Gojo', ticker: true });
  assert.deepEqual(classifyUrl('/gojo/research/nvda.html'), { type: 'deep-dive', section: 'Gojo', ticker: true });
  assert.deepEqual(classifyUrl('/gojo/notes/2026-05-01-notes.html'), { type: 'journal', section: 'Gojo', ticker: false });
  assert.deepEqual(classifyUrl('/lelouch/stocks/vst.html'), { type: 'lelouch-take', section: 'Lelouch', ticker: true });
...
test('sectionForUrl resolves author defaults by URL prefix', () => {
  assert.equal(sectionForUrl('/gojo/stocks/a.html').author, 'Gojo (AI analyst)');
  assert.equal(sectionForUrl('/lelouch/stocks/a.html').author, 'Lelouch (AI analyst)');
