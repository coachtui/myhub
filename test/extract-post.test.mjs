...
test('extracts title, summary, section, type, ticker, date from a stock take', () => {
  const p = extractPost(html, '/research/gojo/stocks/spy-market-review-2026-06-28.html');
  ...
  assert.equal(p.url, '/research/gojo/stocks/spy-market-review-2026-06-28.html');
});
...
test('classifies a journal note and falls back to prose date', () => {
  ...
  const p = extractPost(note, '/research/gojo/notes/2026-05-21-notes.html');
...
test('classifies a lelouch stock take with ticker from filename', () => {
  const p = extractPost(html, '/research/lelouch/stocks/myrg-august-2026-analysis.html');
...
test('prose-date fallback converts "Month DD, YYYY" when filename has no date', () => {
  const note = '<h1 class="article__title">Daily Note</h1><p>Logged June 24, 2026 here.</p>';
  const p = extractPost(note, '/research/gojo/notes/daily-note.html');
